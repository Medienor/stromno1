...
def get_electricity_api_url():
    ...
    return f"https://www.hvakosterstrommen.no/api/v1/prices/{today}_NO1.json"
...
def get_reservoir_data():
    ...
    if response.status_code == 200:
        data = response.json()
        for item in data:
            if item["omrType"] == "EL" and item["omrnr"] == 1:
                return item
    print(f"Failed to retrieve reservoir data. Status code: {response.status_code}")
    return None

webflow_url = "https://api.webflow.com/v2/collections/66a57091a5a25e35ec0ad403/items/66a5724e739b3e60c03dbda3/live"
